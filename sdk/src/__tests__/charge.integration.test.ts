...
function createTestSigner(): ClientSigner {
    return async (txns: Uint8Array[], indexesToSign?: number[]) => {
        const { decodeTransaction } = await import('@algorandfoundation/algokit-utils/transact');
...
describe('Server: challenge issuance', () => {
    ...
    it.skipIf(!hasKey)('challenge includes suggestedParams', async () => {
        ...
        expect(requestData.methodDetails?.suggestedParams).toHaveProperty('firstValid');
        expect(requestData.methodDetails?.suggestedParams).toHaveProperty('lastValid');
        expect(requestData.methodDetails?.suggestedParams).toHaveProperty('genesisHash');
        expect(requestData.methodDetails?.suggestedParams).toHaveProperty('genesisId');
    });

    it.skipIf(!hasKey)('challenge includes feePayerKey when signer configured', async () => {
        const mppx = createMppx({
            recipient: recipientAddress,
            signer: feePayerSigner,
            signerAddress: feePayerAddress,
...
describe('Encoding: transaction roundtrip', () => {
    it.skipIf(!hasKey)('encodeTransactionRaw roundtrip preserves group ID and sender', async () => {
        const { decodeTransaction } = await import('@algorandfoundation/algokit-utils/transact');
...
describe('End-to-end: full ALGO payment flow (TestNet)', () => {
    it.skipIf(!hasKey)('completes a self-payment via pull mode', async () => {
        ...
        const challengeRequest = challenge.request as {
            amount: string;
            methodDetails: {
                suggestedParams: { firstValid: number; lastValid: number; genesisHash: string; genesisId: string };
                feePayer: boolean;
                feePayerKey: string;
                reference: string;
            };
        ...
        const successResponse = (result2 as { status: 200; withReceipt: (r: Response) => Response }).withReceipt(
        ...
        ) as { method: string; reference: string; status: string; timestamp: string };
        ...
        console.log(`  https://lora.algokit.io/testnet/transaction/${receipt.reference}`);
    }, 60_000);
});
